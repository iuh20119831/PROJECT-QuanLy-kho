...
class DonMua{
    ...
    static function donmuatheothang () {
        $list =[];
        $db =DB::getInstance();
        $reg = $db->query('SELECT MONTH(NgayMua) AS Thang, SUM(ThanhTien) AS TongDoanhThuMua FROM DonMua WHERE YEAR(NgayMua) = YEAR(CURDATE()) GROUP BY MONTH(NgayMua)');
        foreach ($reg->fetchAll() as $item){
            $list[$item['Thang']] = $item['TongDoanhThuMua'];
        }
        return $list;
    }
}
