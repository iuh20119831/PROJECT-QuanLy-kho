...
class DonBan{
    ...
    static function donbantheothang () {
        $list =[];
        $db =DB::getInstance();
        $reg = $db->query('SELECT MONTH(NgayBan) AS Thang, SUM(Tong) AS TongDoanhThuBan FROM DonBan WHERE YEAR(NgayBan) = YEAR(CURDATE()) GROUP BY MONTH(NgayBan)');
        foreach ($reg->fetchAll() as $item){
            $list[$item['Thang']] = $item['TongDoanhThuBan'];
        }
        return $list;
    }
}
